...
ADVANCED_MODEL = "google/gemini-2.0-flash-exp:free"   # Higher capacity model for complex queries
FALLBACK_MODEL = "meta-llama/llama-3.2-3b-instruct:free"       # Fallback model if others fail
...
async def generate_streaming_response(client, url, headers, json_data):
    ...
    async with client.stream("POST", url, headers=headers, json=json_data, timeout=90.0) as response:
                    ...
                    for line in complete_lines:
                        if line.startswith('data:') and line.strip() != 'data: [DONE]':
                            try:
                                ...
                                logger.warning(f"Incomplete JSON chunk received: {line}. Error: {e}")
...
async def chat(message: Message):
    ...
    base_system_prompt = "You are AgriBot, a helpful assistant specialized in agricultural advice and data interpretation. Provide insightful and actionable farming recommendations based on location data, soil conditions, and agricultural metrics."
    
    if message.context_type == "farm_selected":
        system_prompt = f"{base_system_prompt} The user has selected a farm location. Analyze this geographical context to provide location-specific agricultural insights such as suitable crops, regional climate patterns, and local best practices. When specific values are provided (like NDVI, soil moisture, etc.), interpret them in practical terms for the farmer."
    elif message.context_type == "data_loaded":
        system_prompt = f"{base_system_prompt} The user has loaded farm data. Analyze the provided metrics and offer meaningful interpretations. For example, explain what the values mean for crop health, soil conditions, or irrigation needs. Translate technical data into practical farming advice. If coordinates are provided, consider regional agricultural patterns for that location."
    else:
        system_prompt = f"{base_system_prompt} Provide general farming information and encourage the user to select a farm location on the map for more tailored advice. Explain the benefits of location-specific agricultural insights."

    # Select appropriate model based on the query
    selected_model = select_model(message.text, message.context_type)
    logger.info(f"Selected model: {selected_model} for context type: {message.context_type}")
    ...
    try:
        if message.use_streaming:
            ...
            logger.info(f"Sending streaming request to OpenRouter with models: {models_to_try}")
            ...
            logger.info(f"Sending non-streaming request to OpenRouter with models: {models_to_try}")
