...
export interface GeoJSONFeature<G, P = Record<string, any>> { // Added default for P
  type: 'Feature';
  geometry: G;
  properties: P;
}
...
export async function queryValueByGeometry(
  ...
  geom: ApiGeomType
): Promise<any> { // Return type is any for now, can be refined if response structure is known
  ...
  try {
    ...
    console.error('Error in queryValueByGeometry:', error.response ? error.response.data : error.message);
