  ...
  scope.run(() => {
    watch(
      ...
      async (newValues, oldValues) => {
        console.log('[useReactiveMapDataManager] Watcher triggered. New values:', JSON.parse(JSON.stringify(newValues)), 'Old values:', JSON.parse(JSON.stringify(oldValues)));
        ...
        if (productId && date && coordinates) {
          ...
          if (
            ...
            console.log('[useReactiveMapDataManager] Conditions met. Calling loadDataForClickedPointViaPolygon for pinned point:', JSON.parse(JSON.stringify(coordinates)));
            await productStore.loadDataForClickedPointViaPolygon(coordinates.longitude, coordinates.latitude);
          } else {
            console.log('[useReactiveMapDataManager] Watched values changed, but key data (productId, date, coordinates) for pinned point remains the same. No API call.');
          }
        } else {
          console.log('[useReactiveMapDataManager] Not all conditions met for reactive data load (productId, date, or coordinates missing).');
        }
      },
