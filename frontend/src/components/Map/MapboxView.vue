...
import { inject, onMounted, onBeforeUnmount, useAttrs, watch, shallowRef } from 'vue';
...
onMounted(() => {
  ...
  if (!mapContainer) {
    console.error("[MapboxView.vue] Map container element with ID 'map' not found in the DOM.");
    return;
  }

  if (!props.accessToken) {
    console.error('[MapboxView.vue] Mapbox access token is required but was not provided.');
    return;
  }
  mapboxgl.accessToken = props.accessToken;

  if (!viewState) {
    console.error('[MapboxView.vue] ViewState was not injected. Map cannot be initialized without it.');
    return;
  }
  ...
  initialMapInstance.on('load', () => {
    ...
    if (initialStyleName) {
      ...
      switch (initialStyleName.toLowerCase()) {
        ...
        default:
          console.warn(
            `[MapboxView.vue] Unrecognized initial map style name: '${initialStyleName}'. Could not map to a known basemapId. Style URL: ${currentStyle?.url || 'N/A'}`
          );
      }
      if (initialBasemapId) {
        mapStore.selectedBasemap = initialBasemapId;
      } else {
        console.warn(`[MapboxView.vue] Could not determine a basemapId for style name: '${initialStyleName}'. Check style name to basemapId mappings.`);
      }
    } else {
      console.warn('[MapboxView.vue] Initial map style name is undefined (props.mapStyle might be missing or style has no name). Basemap synchronization might be affected.');
    }
  ...
  initialMapInstance.on('error', (errorEvent) => {
    console.error('[MapboxView.vue] A Mapbox GL error occurred:', errorEvent?.error);
  });
});
 ...
 */
watch(
  viewState!, // Non-null assertion as viewState is checked in onMounted
  (newState) => {
    if (map.value && newState) {
      // Use flyTo for smoother transitions or jumpTo for immediate changes
      map.value.jumpTo({
  ...
  { deep: true } // Deep watch is necessary for objects
);
...
watch(
  ...
  (newBasemapId) => {
    if (map.value && newBasemapId) {
      ...
      if (styleUrl) { // Only attempt to set style if a valid URL was determined
