...
onMounted(() => {
  ...
  map.value.on('load', () => {
    ...
    switch (initialStyleName) {
      ...
      default:
        console.warn(
          `[MapboxView.vue] Unknown initial map style name: ${initialStyleName}. Defaulting to 'dark'.`
        )
    }
    ...
    darkModeMediaQuery.addEventListener(
      'change',
      updateBasemapForSystemPreference
    )
  ...
  (newBasemap) => {
    if (map.value) {
      ...
      switch (newBasemap) {
        ...
        default:
          console.warn(
            `[MapboxView.vue] Unknown basemap: ${newBasemap}. Falling back to 'streets'.`
          )
          styleUrl = 'mapbox://styles/mapbox/streets-v12'
      }
...
<template>
  <div id="map" ref="map" class="w-full h-full absolute top-0 left-0" />
</template>
